...
type Node struct {
	...
	// Agent 配置（由 Master 前端初始化后入库并下发）
	Initialized        bool   `gorm:"type:boolean;default:false" json:"initialized,omitempty"`
	...
	ConsoleBindAddr    string `gorm:"type:varchar(64);default:'127.0.0.1:0'" json:"console_bind_addr,omitempty"`
	...
	PublicIPPool  []PublicIPPool `gorm:"foreignKey:NodeID" json:"public_ip_pool,omitempty"`
	IPv6Prefixes  []IPv6Prefix   `gorm:"foreignKey:NodeID" json:"ipv6_prefixes,omitempty"`
}
